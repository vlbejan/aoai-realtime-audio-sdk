""""
This file commbines the fucntionalities of both client_sample.py and client_sample_no_vad.py.
"""
import asyncio
import os
import sys
...
async def with_azure_openai(audio_file_path: str, instructions_file: str, out_dir: str, use_vad: bool):
    ...
    async with RTClient(url=endpoint, key_credential=AzureKeyCredential(key), azure_deployment=deployment) as client:
        await run(client, audio_file_path, instructions_file, out_dir, use_vad)

async def with_openai(audio_file_path: str, instructions_file: str, out_dir: str, use_vad: bool):
...
if __name__ == "__main__":
    load_dotenv()
    if len(sys.argv) < 4:
        print(f"Usage: python {sys.argv} <audio_file> <instructions_file> <out_dir> <use_vad> [azure|openai]")
        print("If the fifth argument is not provided, it will default to azure")
        sys.exit(1)
    ...
    if provider == "azure":
        ...
        asyncio.run(with_openai(file_path, instructions_file, out_dir, use_vad))