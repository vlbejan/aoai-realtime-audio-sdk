...
async def receive_message_item(item: RTMessageItem, out_dir: str):
    ...
    async for contentPart in item:
        if contentPart.type == "audio":
            ...
            with open(os.path.join(out_dir, f"{VOICE}_{item.id}_{contentPart.content_index}.wav"), "wb") as out:
                audio_array = np.frombuffer(audio_data, dtype=np.int16)
                sf.write(out, audio_array, samplerate=24000)
            with open(
                os.path.join(out_dir, f"{VOICE}_{item.id}_{contentPart.content_index}.audio_transcript.txt"),
                "w",
                encoding="utf-8",
            ) as out:
...
if __name__ == "__main__":
    ...
    VOICE_LIST = ["alloy", "ash", "ballad", "coral", "echo", "sage", "shimmer", "verse"]
